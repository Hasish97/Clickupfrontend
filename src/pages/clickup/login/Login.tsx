...
class Login extends React.Component<{}, {}> {
    ...
    handleLogin = async (event: any) => {
        ...
        try {
            const data = await Axios.post('http://192.168.1.36:8080/api/auth/v1/auth_user', {
                username: this.state.username,
                password: this.state.password
            });

            localStorage.setItem('jwtToken', data.data.result[0].token);
            localStorage.setItem('user', JSON.stringify(data.data.result[0].user));
                                ...
                                <div className="col-md-12">
                                    <Form>
                                        <div className="card-body">
                                            ...
                                            {this.state.buttondisbled ? (
                                                <Button disabled className="btn btn-block btn-primary mb-4 rounded-pill">
                                                    <span className="spinner-border spinner-border-sm mr-1" role="status" />
                                                    Loading...
                                                </Button>
                                            ) : (
                                                <button onClick={this.handleLogin} className="btn btn-block btn-primary mb-4 rounded-pill">
                                                    Signin
                                                </button>
                                            )}
