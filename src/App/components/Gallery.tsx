...
interface IGalleryProps extends React.HTMLAttributes<Element> {
  videos?: string[];
  texts?: string[];
  images?: Array<{ src: string; useForDemo?: boolean; thumbnail: string; orientation?: string; caption?: string }>;
  backdropClosesModal?: any;
  theme?: any;
  showThumbnails?: any;
  subheading?: any;
  heading?: any;
...
class Gallery extends React.Component<IGalleryProps, GalleryState> {
  ...
  openLightbox(event: React.MouseEvent<HTMLElement>, index: number) {
  ...
  handleClickItem() {
    const items = this.state.itemType === 'videos' ? this.props.videos : this.props.images;
    if (this.state.currentItem === items!.length - 1)
      return;
    this.gotoNext();
  }
  renderGallery() {
    ...
    return (
      <div className="section">
        ...
        <Lightbox
          ...
          items={{ type: this.state.itemType, items: this.state.itemType === 'videos' ? this.props.videos : this.props.images }}
